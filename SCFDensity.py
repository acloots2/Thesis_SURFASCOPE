...
def xc_pot(dens, sym = True):
    e_xc = np.multiply(dens,ueg.e_total(dens, 2))
...
def sc_cycle(z_vec, dens0, bg_dens, d_slab, dens_3d, point_dens, dimension = 3):
    fig_pot = go.Figure()
    fig_dens = go.Figure()
    ...
    i = 0
    npnt = len(dens0)
    while np.sum(np.abs(dens0-dens2)/npnt)>dens_tot*1e-8 and i<15:
    ...
    size=5,
        ...
        fig_pot.add_trace(go.Scatter(x = tools.center_z(z_vec), y = np.real(vxc_0), name = "xc Cycle "+str(2+i), mode = "lines+markers",marker=dict(
    size=5,
    ),))
