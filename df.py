...
class DielectricFunction:
    ...
    def calculate_chi0(self, q_c):
        ...
        if self.name:
            ...
            name = self.name + '.pckl' 
          #  name = self.name + ('%+d%+d%+d-chi0.pckl' % tuple((q_c * kd.N_c).round())) 
    ...
    def get_dielectric_matrix(self, xc='RPA', q_c=[0, 0, 0],
                              direction='x', symmetric=True,  #BM : symmetric=True normally
    ...
    def get_eigenmodes(self, q_c=[0, 0, 0], w_max=None, name=None,
        ...
        weight = np.zeros([Nw,nG],dtype=complex) #BM
        vm=np.zeros([Nw,nG],dtype=complex) #BM
        nm=np.zeros([Nw,nG],dtype=complex) #BM
       # epsn_LF=np.zeros([Nw,nG], dtype=complex) #BM
       # epsn_NLF=np.zeros([Nw,nG], dtype=complex) #BM
        ...
        #        continue
            ...
            weight[i]=vec[0,:]*(np.transpose(vec_dual[:,0])) #BM the weight of each mode
            vm[i]=vec[0,:] #BM Mean value of potential and charges
            nm[i]=np.transpose(vec_dual[:,0]) 
           # epsn_LF[i]=1./eig[i]*vec[0,:]*np.conj(np.transpose(vec_dual[:,0])) #BM:Calculate the individual permittivity of each mode 
           # epsn_NLF[i]=eig[i]*vec[0,:]*np.conj(np.transpose(vec_dual[:,0])) #BM:Calculate the individual permittivity of each mode
            ...
            for k in [k for k in range(nG)
               ...
               # n_temp = np.dot(factor, vec[:, k] * coef_G)
                n_temp = np.dot(factor,np.conj(np.transpose(vec_dual[k,:]))) #BM
            ...
            if i==10:                                               # Adding a field map to save (e.g. frequency n°79 and mode n°2)
                qG = pd.get_reciprocal_vectors(add_q=True)
                coef_G = np.diagonal(np.inner(qG, qG)) / (4 * pi)
                qGr_R = np.inner(qG, r.T).T
                factor = np.exp(1j * qGr_R)
                v_res = np.dot(factor, vec[:, 1])
                n_res = np.dot(factor, np.conj(np.transpose(vec_dual[1, :])))
 
        #    if i==179:                                               # Adding a field map to save (e.g. frequency n°179 and mode n°9)
        #        qG = pd.get_reciprocal_vectors(add_q=True)
        #        coef_G = np.diagonal(np.inner(qG, qG)) / (4 * pi)
        #        qGr_R = np.inner(qG, r.T).T
        #        factor = np.exp(1j * qGr_R)
        #        v_res2 = np.dot(factor, vec[:, 9])
        #        n_res2 = np.dot(factor, np.conj(np.transpose(vec_dual[9, :])))


        kd = self.chi0.calc.wfs.kd
        if name is None and self.name:
            name = self.name + 'eigenmodes.pckl' 
           # name = self.name + ('%+d%+d%+d-eigenmodes.pckl' % tuple((q_c * kd.N_c).round()))
        elif name:
            name = name + 'eigenmodes.pckl'   
        ...
        if eigenvalue_only:
            ...
            pickle.dump((r * Bohr, w_w, eig, omega0, eigen0,
                         v_ind, n_ind, weight,v_res,n_res,vm,nm), open(name, 'wb'), #BM: added weight,v_res,n_res
                        pickle.HIGHEST_PROTOCOL)
            return r * Bohr, w_w, eig, omega0, eigen0, v_ind, n_ind, weight,v_res,n_res,vm,nm  #BM: added weight,v_res,n_res
